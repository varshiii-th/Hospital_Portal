...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!empty($_POST["prof_id"]) && !empty($_POST["password"])){
        $prof_id = $_POST["prof_id"];
        $password = $_POST["password"];
        $sql = "SELECT * FROM professor WHERE  professor_id= '$prof_id' ";
        $sql3 = "SELECT * FROM professor WHERE professor_id= '$prof_id'AND password = '$password'";
        $result = $conn->query($sql);
        $result3 = $conn->query($sql3);
        if ($result3->num_rows == 1) {
            $_SESSION['prof_id']=$prof_id;
            $sql5 = "SELECT patient_id FROM patient 
            WHERE  id = '$prof_id' AND patient_role='professor'";
            ...
            echo "✅ Login successful! Welcome, " . $prof_id;
            echo '<meta http-equiv="refresh" content="3;url=prof_dashboard.php">';
        } else if ($result->num_rows == 0) {
            echo "no account";
            echo '<meta http-equiv="refresh" content="3;url=prof_signup.php">';
        } 
        else {
            echo "❌ Invalid password. Redirecting to login...";
            echo '<meta http-equiv="refresh" content="3;url=prof_login.php">';
        }
    }
    else {
        echo "⚠️ Please enter both Professor_id and Password.";
    }
}
?>
<!DOCTYPE html>
...
<body>
    <form action="prof_login.php" method="post">
        <input type="text" id="prof_id" name="prof_id" placeholder="professor_id">
        <br>
        <input type="password" id="password" name="password" placeholder="password">
        <br>
        <input type="submit" value="Submit">
           <a href='../professor.php' class='back-link'>Back_To_Index</a>
