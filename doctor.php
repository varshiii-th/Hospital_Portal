...
?>
<?php
if(isset($_POST['docentrytype'])){
    $entry=$_POST['docentrytype'];
    if ($entry==0){
        header("Location:doctor/doc_signup.php");
        exit();
    }
    else if($entry==1){
...
main {
    ...
    margin: 0 auto;
    padding: 30px;
    background:rgba(236, 230, 230,0.00);
    border-radius: 12px;
        ...
        <main>
            <section>
                <form action="doctor.php" method="post">
                    <h3><b>If you are a new Doctor, please select Signup; else, Login</b></h3>
                    <label>
                        <input type="radio" value="0" name="docentrytype" required> Signup
                    </label>
                    <label>
                        <input type="radio" value="1" name="docentrytype"> Login
                    </label>
